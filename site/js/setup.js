...
document.addEventListener('DOMContentLoaded', function() {
    try {
        ...
        document.addEventListener("sessionPayment", function() {
            ...
            let customChip = formatMoneyToFloat(curChip.innerText) + event.chip;
        ...
        let moneyInput = document.getElementById("custom-chip");
...
function updateSendMoneyBtn() {
    const isFilledCard = document.getElementById("payment-cc-number").value.length === 19;
    const isFilledMonth = isCorrectMonth(document.getElementById("payment-cc-month").value);
    const isFilledYear = document.getElementById("payment-cc-year").value > 0;
    const isFilledCv2 = document.getElementById("payment-cc-cv2").value.length === 3;
    const btn = document.getElementById("payment-cc-button");
...
function parseCustomChip(event, input) {
    ...
    let filtered = innerContent.replace(/[^0-9.]/g, "");
...
function formatMoney(amount, decimalCount=2, decimal='.', thousands=' ') {
    ...
    const fraction = (j ? i.substr(0, j) + thousands : '') + i.substr(j).replace(/(\d{3})(?=\d)/g, "$1" + thousands);
    if (amount - i <= 0.01)
...
function updateAccumulatedMoney(fundInfo) {
    const money = getAccumulatedMoney();
...
function handlerPaymentButtons(fundInfo) {
    ...
    for (const [btnId, bankName] of Object.entries(bankButtons)) {
        ...
        elem.addEventListener("click", function () {
            ...
            const paymentInfo = getPaymentInfo(bankName);
...
function updateJarGrid(maxValue) {
    ...
    const temp = calcGridDestribution(maxValue, gridId.length);
...
function calcGridDestribution(maxValue, gridScaleCount) {
    ...
    for (let i = 1; i < gridScaleCount - 1; i++)
        out[i] = out[i - 1] + step;
